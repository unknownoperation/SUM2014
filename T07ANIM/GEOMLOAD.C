...
*/
static VOID LoadMaterials( ok2GEOM *G, CHAR *FileName )
{
  FILE *F;
  ok2MATERIAL DefMat, Mat;
...
BOOL OK2_GeomLoad( ok2GEOM *G, CHAR *FileName )
{
  ...
  size = 
    ...
    sizeof(ok2UV) * vtn +                     /*   текстурные координаты vtn */
  ...
  OK2_DefaultColor = ColorSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT minv, maxv, j;
    ok2PRIM prim;
    ...
    OK2_PrimCreate(&prim, OK2_PRIM_TRIMESH, vn, fn * 3);
    ...
    OK2_GeomAddPrim(G, &prim);
  }
  /* освобождаем память из-под прочитанных данных */
  free(ReadV);
  return TRUE;
} /* End of 'OK2_GeomLoad' function */
